...
function begin_play(){
	...
	if(isEmpty(player1_id.value) || isEmpty(player2_id.value)){
		alert("Two player game, both field are mandatory");
	}
	//if(move_text_id.value.length == 0 || player1_id.value.length == 0 || player2_id.value.length == 0 ){
	//	return; 
	//}
	else{
...
function reset_play(){
	...
	document.getElementById("player1_id").placeholder = "player1_id";
	document.getElementById("player2_id").placeholder = "player2_id";
	document.getElementById("player1_id").value = "";
	document.getElementById("player2_id").value = "";
	document.getElementById("move_text_id").placeholder = "Cell Number for Example: A1";
...
function play() {
	...
	var move_text_id = document.getElementById("move_text_id").value;
	if( move_text_id != "A1" && move_text_id != "A2" && move_text_id != "A3" && move_text_id != "B1" && move_text_id != "B2" && move_text_id != "B3" && move_text_id != "C1" && move_text_id != "C2" && move_text_id != "C3"){
        
        alert("Invalid move.");
        return
    }
    //check if space is taken
    if(move_text_id != document.getElementById(move_text_id).innerHTML){
        
        alert("Invalid move. Space Taken");
        return
    }
    else{
    	...
    	//player 1 turn
        if(whose_move() == 1){
            document.getElementById(move_text_id).innerHTML = "X";
            
                                    
            location = this.table_ids.indexOf(move_text_id);
        ...
        //player 2 turn
            ...
            document.getElementById(move_text_id).innerHTML = "O";
            

            location = this.table_ids.indexOf(move_text_id);
            this.board_state[location] = 0;
    ...
    else if(board_state[0] == board_state[3] && board_state[3] == board_state[6] && board_state[0] != -1){
        if(turn == 0){
            alert("X has won!")
            reset_play();
            return;
    ...
    else if(board_state[2] == board_state[5] && board_state[5] == board_state[8] && board_state[2] != -1){
        if(turn == 0){
            alert("X has won!")
            reset_play();
            return;
    ...
    else if(board_state[0] == board_state[4] && board_state[4] == board_state[8] && board_state[0] != -1){
        if(turn == 0){
            alert("X has won!")
            reset_play();
            return;
    ...
    else if(board_state[2] == board_state[4] && board_state[6] == board_state[4] && board_state[2] != -1){
        if(turn == 0){
            alert("X has won!")
            reset_play();
            return;
